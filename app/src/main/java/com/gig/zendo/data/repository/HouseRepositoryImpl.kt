...
class HouseRepositoryImpl @Inject constructor(
    ...
    override suspend fun addHouse(house: House): UiState<Unit> {
        return try {
            val houseId = house.id.ifEmpty { firestore.collection("houses").document().id }
            firestore.collection("houses").document(houseId).set(house.copy(id = houseId)).await()
    ...
    override suspend fun getHouses(): UiState<List<House>> {
        return try {
    ...
    override suspend fun deleteHouse(houseId: String): UiState<Unit> {
        return try {
            firestore.collection("houses").document(houseId).delete().await()
            UiState.Success(Unit)
        } catch (e: Exception) {
    ...
    override suspend fun updateHouse(house: House): UiState<Unit> {
        return try {
            ...
            firestore.collection("houses").document(houseId).set(house).await()
            UiState.Success(Unit)
        } catch (e: Exception) {
